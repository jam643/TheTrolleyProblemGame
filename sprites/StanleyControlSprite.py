...
class StanleyControlSprite(StanleyControl, ControlSpriteBase):
    ...
    def draw(self, screen):
        if self.nearest_pose and self.car_front_axle:
            draw_arrow(screen, self.nearest_pose, 2, self.sim_to_real, WHITE)
            draw_arrow(screen, self.car_front_axle, 2, self.sim_to_real, WHITE)
            draw_arrow(
                screen,
                math.Pose(
                    self.car_front_axle.x,
                    self.car_front_axle.y,
                    self.nearest_pose.theta,
                ),
                2,
                self.sim_to_real,
                WHITE,
            ...
            draw_arrow(
                ...
                math.Pose(
                    self.nearest_pose.x,
                    self.nearest_pose.y,
                    self.nearest_pose.theta + np.pi / 2,
                ),
                self.cte,
                self.sim_to_real,
                WHITE,
            ...
            self.plot_manager.plots["e1"].update(
                1e3 * self.sim_to_real.time_s, self.cte
            )
            self.plot_manager.plots["e2"].update(
                1e3 * self.sim_to_real.time_s, np.rad2deg(self.theta_e)
            )
