...
class LQRSprite(ControlSpriteBase):
    def __init__(self, tracker: LQRPathTrackerBase, glob_to_screen: GlobToScreen, screen: pygame.Surface):
    ...
    def draw(self, screen):
        if self.tracker.nearest_pose:
            pygame.draw.circle(screen, WHITE, self.glob_to_screen.get_pxl_from_glob(self.tracker.car_ref_pnt.to_vect2()),
                               4)
            pygame.draw.circle(screen, WHITE, self.glob_to_screen.get_pxl_from_glob(self.tracker.nearest_pose.to_vect2()), 4)

            draw_arrow(screen, self.tracker.nearest_pose, 2, self.glob_to_screen, WHITE)

            # draw cte line
            draw_arrow(screen, math.Pose(self.tracker.nearest_pose.x, self.tracker.nearest_pose.y, self.tracker.nearest_pose.theta + np.pi / 2),
                       self.tracker.cte, self.glob_to_screen, WHITE)
            ...
            try:
                if -1000 < rad * self.glob_to_screen.pxl_per_mtr < 1000:
                    center = self.glob_to_screen.get_pxl_from_glob(
                        math.add_body_frame(self.tracker.nearest_pose, math.Pose(0, rad, 0)).to_vect2())
                    arc_angles = np.array([-self.tracker.nearest_pose.theta - d / rad * np.sign(rad),
                                           -self.tracker.nearest_pose.theta + d / rad * np.sign(rad)]) + np.pi / 2 * np.sign(rad)
                    ...
                    pygame.draw.line(screen, WHITE, self.glob_to_screen.get_pxl_from_glob(
                        math.add_body_frame(self.tracker.nearest_pose, math.Pose(-d, 0, 0)).to_vect2()),
    ...
    def draw_plots(self, screen: pygame.Surface):
        if self.tracker.nearest_pose:
            self.plot_manager.plots["e1"].update(1e3 * self.glob_to_screen.time_s, self.tracker.cte)
            self.plot_manager.plots["e2"].update(1e3 * self.glob_to_screen.time_s, np.rad2deg(self.tracker.theta_e))
            self.plot_manager.plots["e1d"].update(1e3 * self.glob_to_screen.time_s, self.tracker.cte_dot)
            self.plot_manager.plots["e2d"].update(1e3 * self.glob_to_screen.time_s, np.rad2deg(self.tracker.theta_e_dot))
            self.plot_manager.plots["solvetime"].update(1e3 * self.glob_to_screen.time_s, self.tracker.solvetime_ms)
