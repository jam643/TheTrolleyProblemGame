...
class LowLevelControlFactory:
    ...
    def __init__(self, sim_to_real, screen, vehicle: Vehicle):
        ...
        self.plot_manager.add_plot(
            "steer",
            PgPlot(
                math.Point(width, height), "STEER [DEG]", screen, [-40, 40], 3000, 20
            ),
        )
        self.plot_manager.add_plot(
            "speed",
            PgPlot(
                math.Point(width, height), "SPEED (COG) [M/S]", screen, [0, 30], 3000, 5
            ),
        )

    def update(self, steer_rate, steer_desired, vel, dt):
        self.vehicle_state = self._motion_model_map[self._current_model_type].update(
            self.vehicle_state, steer_rate, steer_desired, vel, dt
        )
    ...
    def draw(self, screen: pygame.Surface):
        ...
        if self._draw_plots:
            self.plot_manager.plots["steer"].update(
                1e3 * self.sim_to_real.time_s,
                np.rad2deg(self.vehicle_state.state_cog.delta),
            )
            self.plot_manager.plots["speed"].update(
                1e3 * self.sim_to_real.time_s, self.vehicle_state.vel_cog_mag
            )
            self.plot_manager.draw_plots(screen)

    def _init_menus(self):
        # init menus/frames
        self.ll_control_menu = menu_config(
            title="LOW LEVEL CONTROL", screen=self.screen
        )
        f_ll_control_menu = v_frame(self.screen, self.ll_control_menu)
        self._speed_control_menu = menu_config(
            title="SPEED CONTROL", screen=self.screen
        )
        f_speed_control_menu = v_frame(self.screen, self.ll_control_menu)
        self._steer_control_menu = menu_config(
            title="STEER CONTROL", screen=self.screen
        )
        f_steer_control_menu = v_frame(self.screen, self._steer_control_menu)

        f_ll_control_menu.pack(
            self.ll_control_menu.add.button("SPEED CONTROL", self._speed_control_menu)
        )
        f_ll_control_menu.pack(
            self.ll_control_menu.add.button("STEER CONTROL", self._steer_control_menu)
        )
        f_ll_control_menu.pack(
            self.ll_control_menu.add.button("BACK", pygame_menu.events.BACK)
        )

        f_speed_control_menu.pack(
            self._vehicle_param_menu.add.range_slider(
                "LF",
                self.vehicle_state.params.lf,
                (0.1, 5),
                0.1,
        ...
        f_speed_control_menu.pack(
            self._vehicle_param_menu.add.range_slider(
                "MAX STEER",
                np.rad2deg(self.vehicle_state.params.delta_max),
                (0, 90),
                5,
        ...
        f_steer_control_menu.pack(
            self._steer_control_menu.add.dropselect(
                "INTEGRATION",
                [
        ...
        f_steer_control_menu.pack(
            self._steer_control_menu.add.dropselect(
                "MODEL",
                [
                    ("DYNAMIC", MotionModelType.dynamic_model),
        ...
        f_steer_control_menu.pack(
            self._steer_control_menu.add.button("BACK", pygame_menu.events.BACK)
        )
    ...
    def reset_init_pose(self):
        pose_init = self.sim_to_real.get_pose_glob_from_pxl(
            math.Pose(self.screen.get_width() / 4, self.screen.get_height() / 2, 0)
        )
        vel_init = 10
        self.vehicle_state = (
            Vehicle(params=Vehicle.Params())
            .build_pose(pose_init)
            .build_vel(vel_init, 0)
        )
        self.car_sprite = CarSprite(self.sim_to_real, self.vehicle_state.params)
        return self.vehicle_state
