...
class VehicleFactory:
    ...
    def __init__(
        ...
        sim_to_real,
        screen,
        model_type: MotionModelType = MotionModelType.dynamic_model,
        ...
        self.car_sprite = CarSprite(sim_to_real, self.vehicle_state.params)

        self._motion_model_map = {
            MotionModelType.dynamic_model: CartesianDynamicBicycleModel(),
        ...
        self.plot_manager.add_plot(
            "steer",
            PgPlot(
                math.Point(width, height), "STEER [DEG]", screen, [-65, 65], 3000, 20
            ),
        )
        self.plot_manager.add_plot(
            "steer rate",
    ...
    def draw(self, screen: pygame.Surface):
        ...
        if self._draw_plots:
            ...
            self.plot_manager.plots["steer rate"].update(
                1e3 * self.sim_to_real.time_s,
    ...
    def _init_menus(self):
        ...
        self._vehicle_param_menu = menu_config(
            title="VEHICLE PARAM", screen=self.screen
        )
        ...
        f_vehicle_menu.pack(
            self.vehicle_menu.add.button("RESET VEHICLE", action=self.reset_init_pose)
        )
        f_vehicle_menu.pack(
            self.vehicle_menu.add.button(
                "DOCS",
                action=lambda: self._subprocess_list.append(
                    subprocess.Popen(
                        [
                            ...
                            "./dynamics/docs/VehicleMotionModel.ipynb",
                        ],
        ...
        f_vehicle_menu.pack(
            self.vehicle_menu.add.toggle_switch(
                "SHOW PLOTS",
        ...
        f_motion_model_menu.pack(
            self._motion_model_menu.add.dropselect(
                "INTEGRATION",
                [
    ...
    def reset_init_pose(self):
        pose_init = self.sim_to_real.get_real_from_sim(
