...
class WallFactory:
    def __init__(
        ...
        self.wall_generator = wall_generator.add_callback(self._gen_wall, sim_to_real)
    ...
    def update(self, car_sprite: CarSprite):
        ...
        try:
            wall_collided = pygame.sprite.spritecollideany(
                car_sprite,
                self.wall_sprites,
                lambda car, wall: pygame.sprite.collide_mask(car, wall),
            )
            door_collided = pygame.sprite.spritecollideany(
                car_sprite, self.door_sprites
            )
                ...
                if not car_sprite.is_colliding:
                    ...
                    if self.wall_counter >= self.n_walls:
                        ...
                        if idx < len(self.wall_sprites.sprites()) - 1:
                            [
                                door.kill()
                                for door in self.door_sprites.sprites()[idx + 1 :]
                            ]
                            [
                                wall.dissolve()
                                for wall in self.wall_sprites.sprites()[idx + 1 :]
                            ]
                door_collided.kill()
        except:
            return False
    ...
    def draw(self, screen: pygame.Surface):
        ...
        text.message_to_screen(
            text=f"{self.wall_counter}/{self.n_walls} WALLS",
            screen=screen,
            fontsize=20,
            color=text.WHITE,
            pose=pygame.Vector2(0.01, 0.0),
            hor_align=text.HorAlign.LEFT,
